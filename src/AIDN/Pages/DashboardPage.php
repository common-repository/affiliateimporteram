...
if (!class_exists('AIDN_DashboardPage')) {

    class AIDN_DashboardPage extends WP_List_Table
    {

        public $type = "";
        /**
         * @var AIDN_AbstractConfigurator
         */
        public $api;
        /**
         * @var AIDN_AbstractLoader
         */
        public $loader;
        ...
        {
            parent::__construct();
            $this->api = aidn_get_api($type);

            if ($this->api && $this->api->isInstaled()) {
                $this->type = $this->api->getType();
                $this->loader = aidn_get_loader($this->type);
        ...
        public function render()
        {
            ...
            if (is_plugin_active('woocommerce/woocommerce.php')) {
                do_action('aidn_befor_dashboard_render', $this->api);

                aidn_api_enqueue_style($this->api);
                ...
                $this->link_categories = AIDN_Utils::getCategoriesTree();

                $activePage = 'add';
                include AIDN_ROOT_PATH . '/layout/toolbar.php';

                do_action('aidn_dashboard_render', $this);

                do_action('aidn_after_dashboard_render', $this);
            }
        }
        ...
        public function get_columns()
        {
            ...
            return apply_filters('aidn_get_dashboard_columns', $columns, $this->api);
        }
        ...
        {
            $sortableColumns = [];
            return apply_filters('aidn_get_dashboard_sortable_columns', $sortableColumns);
        }
        ...
        public function column_default($item, $column_name)
        {
            switch ($column_name) {
                case 'image':
                    $result_data = AIDN_DashboardPage::putImageEdit($item);
                    break;
                case 'info':
                    $actions = array();
                    ...
                    $result_data = AIDN_DashboardPage::putField($item, "title", true, "edit", "Title", "") .
                        AIDN_DashboardPage::putField($item, 'subtitle', true, "edit", "Subtitle", "subtitle-block") .
                        AIDN_DashboardPage::putField($item, 'keywords', true, "edit", "Keywords", "subtitle-block") .
                        AIDN_DashboardPage::putDescriptionEdit($item) .
                        ($cat_name ? "<div>Link to category: $cat_name</div>" : "") .
                        $this->row_actions($actions);
                    break;
                case 'condition':
                    $result_data = isset($item->additional_meta['condition']) ? AIDN_Goods::normalized($item->additional_meta['condition']) : "";
                    break;
                case 'ship_to_locations':
                    $result_data = isset($item->additional_meta['ship_to_locations']) ? AIDN_Goods::normalized($item->additional_meta['ship_to_locations']) : "";
                    break;
                case 'ship':
                    $result_data = (isset($item->additional_meta['ship']) && $item->additional_meta['ship']) ? AIDN_Goods::getNormalizePrice($item->additional_meta['ship']) : "";
                    break;
                default:
                    $result_data = AIDN_DashboardPage::putField($item, $column_name, false);
                    break;
            }

            return apply_filters('aidn_dashboard_column_default', $result_data, $item, $column_name);
        }
        ...
        private function processBulkAction()
        {
            ...
            if ((
                ...
                foreach ($_GET['gi'] as $gi) {
                    $goods = new AIDN_Goods($gi);
                    if ($goods->load() && !$goods->post_id && class_exists('AIDN_WooCommerce')) {

                        $importStatus = $_GET['action'] === 'import' ? 'publish' : 'draft';

                        $res = AIDN_WooCommerce::addPost($goods, ['import_status' => $importStatus]);
                        if ($res["state"] !== "error") {
                            $result_cnt++;
                        }
        ...
        public function prepare_items()
        {
            ...
            if ($this->loader) {
                ...
                if (!$this->loader->hasAccount()) {
                    add_settings_error('aidn_dashboard_error', esc_attr('settings_updated'), 'Account not found. You need configure account on setting page', 'error');
                    $this->show_dashboard = false;
                } else if (!is_plugin_active('woocommerce/woocommerce.php')) {
                    add_settings_error('aidn_dashboard_error', esc_attr('settings_updated'), 'Please install the Woocommerce plugin first.', 'error');
                    $this->show_dashboard = false;
                } else {
                    ...
                    if ($result_cnt) {
                        add_settings_error('aidn_goods_posted', esc_attr('settings_updated'), "$result_cnt products have been loaded to WooCommerce", 'updated');
                    }
                    settings_errors('aidn_goods_posted');

                    if (isset($this->filter['reset']) && $this->filter['reset']) {
                        AIDN_Goods::clearList();
                    }

                    $data = $this->loader->loadListProc($this->filter, $current_page);

                    if ($data["error"]) {
                        add_settings_error('aidn_goods_list', esc_attr('settings_updated'), $data['error'], 'error');
                    }
                    ...
                    if (isset($_GET['orderby']) && function_exists("aidn_sort_by_" . $_GET['orderby'])) {
                        uasort($this->items, "aidn_sort_by_" . $_GET['orderby']);
                    ...
                    $db->query('delete from ' . $db->prefix . AIDN_TABLE_GOODS_ARCHIVE . ' where external_id in (
                    select external_id from ' . $db->prefix . AIDN_TABLE_GOODS . '
                    )');
                    $db->query('insert into ' . $db->prefix . AIDN_TABLE_GOODS_ARCHIVE . '
                    select * from ' . $db->prefix . AIDN_TABLE_GOODS . '
                    ');
                }
            }
        }
        ...
        static public function putImageEdit($item, $content_only = false)
        {
            ...
            foreach ($photos as $photo) {
                $out .= sprintf('<div class="aidn_select_image"><img class="' . ($cur_image === $photo ? "sel" : "") . '" src="%1$s"/></div>', $photo);
            }
