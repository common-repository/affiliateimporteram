 ...
 *
 */
if (!class_exists('AIDN_AbstractLoader')) {

    /**
     * Class AIDN_AbstractLoader
     */
    abstract class AIDN_AbstractLoader
    {

        /**
         * @var AIDN_AbstractAccount
         */
        public $account;
        ...
        /**
         * AIDN_AbstractLoader constructor.
         * @param AIDN_AbstractConfigurator $api
         */
        public function __construct($api)
        {
            $this->api = $api;
            $this->account = aidn_get_account($api->getType());
        }
        ...
        public function loadListProc($filter, $page = 1)
        {
            ...
            foreach ($result['items'] as $key => $item) {
                ...
                $formulas = AIDN_PriceFormula::getGoodsFormula($item);
                if ($formulas) {
                    $item->user_price = AIDN_PriceFormula::applyFormula($item->user_price, $formulas[0]);
                    $item->saveField('user_price', sprintf('%01.2f', $item->user_price));

                    $item = AIDN_PriceFormula::calcRegularPrice($item, $formulas[0]);
                    $item->save('API');
            ...
            $result['items'] = apply_filters('aidn_load_list_item_proc', $result['items'], $filter);
        ...
        public function loadDetailProc(&$goods, $params = array())
        {
            ...
            if ($result['state'] === 'ok') {
                ...
                $result['goods'] = apply_filters('aidn_get_detail_proc', $goods, $params);
                /**
                 * @var AIDN_Goods $resultGoods
                 */
        ...
        public function getDetailProc($productId, $params = array())
        {
            ...
            if ($result['state'] === 'ok') {
                ...
                // update user price by formula
                $formulas = AIDN_PriceFormula::getGoodsFormula($goods);
                if ($formulas) {
                    $goods->user_price = AIDN_PriceFormula::applyFormula($goods->user_price, $formulas[0]);
                    $goods = AIDN_PriceFormula::calcRegularPrice($goods, $formulas[0]);
                }
